...
export default function CustomerCheckoutPage({ demoOrders = [], settings = {}, store = {}, receiverWallet = '' }) {
      ...
      try {
        if (hasSupabaseConfig && supabase && token) {
          ...
          if (data) {
            ...
            setOrder(mapped);
            setCustomerWallet(data.customers?.wallet_address || mapped.customerWallet || '');
            setAvailablePoints(Number(data.customers?.point_balance || 0));
          ...
          const found = demoOrders.find(item => item.checkoutToken === token || item.code === token || item.id === token);
  ...
  async function connectWallet() {
    ...
    try {
      ...
      setCustomerWallet(wallet.address);
      setWalletConnected(true);

      if (hasSupabaseConfig && supabase && wallet.address) {
        const { data } = await supabase.from('customers').select('*').ilike('wallet_address', wallet.address).limit(1).maybeSingle();
        if (data) setAvailablePoints(Number(data.point_balance || 0));
      }
    } catch (error) {
      console.error(error);
  ...
  const subtotal = useMemo(() => {
    if (!order) return 0;
    if (Number(order.subtotal || 0) > 0) return Number(order.subtotal || 0);
    return (order.items || []).reduce((sum, item) => sum + Number(item.total || item.unitPrice * item.qty || 0), 0);
  }, [order]);
  const taxAmount = Math.round(subtotal * taxRate / 100);
  const totalBeforePoints = subtotal + taxAmount;
  const maxDiscountRaw = Math.floor(totalBeforePoints * 0.2);
  const redeemPoints = usePoints ? Math.min(availablePoints, Math.floor(maxDiscountRaw / 100)) : 0;
  ...
  async function payWithArc() {
    ...
    setStatus('paying');
    try {
      const wallet = walletConnected && customerWallet ? { address: customerWallet } : await connectArcWallet();
      setCustomerWallet(wallet.address);
      setWalletConnected(true);
  ...
  if (loading) {
    return <main className="customer-checkout"><section className="checkout-public-card"><RefreshCw className="spin" /> Loading checkout...</section></main>;
  }
    ...
    <main className="customer-checkout">
      <section className="checkout-public-card">
        ...
        <div className="checkout-invoice-title">
          ...
          <span className={`badge ${status === 'paid' ? 'ok' : 'warn'}`}>{status === 'paid' ? 'Paid' : 'Pending'}</span>
        </div>
        ...
        <table className="data-table public-items">
          ...
          <tbody>
            ...
            {!(order.items || []).length && <tr><td colSpan="6" className="empty-row">No invoice items found.</td></tr>}
          </tbody>
        </table>
        ...
        <div className="public-summary-grid">
          ...
          <p><span>Available points</span><strong>{availablePoints} pts</strong></p>
        ...
        {!walletConnected ? (
          <button className="primary full public-pay-button" type="button" onClick={connectWallet}><Wallet size={18} /> Connect Wallet</button>
        ) : (
          <>
            <div className="connected-public-wallet"><Wallet size={18} /> Connected wallet: <strong>{shortAddress(customerWallet)}</strong></div>
            <label className={`check-line ${availablePoints <= 0 ? 'muted' : ''}`}>
              <input type="checkbox" disabled={availablePoints <= 0 || status === 'paid' || status === 'paying' || status === 'confirming'} checked={usePoints} onChange={event => setUsePoints(event.target.checked)} />
              Use loyalty points before signing payment
            </label>
            <p className="helper-text">New loyalty points will be earned from the actual paid amount: <strong>{earnedPoints} pts</strong>.</p>
            <button className="success full public-pay-button" type="button" disabled={status === 'paying' || status === 'confirming' || status === 'paid'} onClick={payWithArc}>
              {status === 'paying' || status === 'confirming' ? <RefreshCw className="spin" size={16} /> : <CheckCircle2 size={16} />}
              {status === 'paid' ? 'Payment Confirmed' : status === 'confirming' ? 'Waiting for Arc finality...' : 'Pay with Arc USDC'}
            </button>
          </>
        )}
        ...
        {errorMessage && <p className="error-text">{errorMessage}</p>}
        {txHash && (
          <a className="explorer-link" href={arcScanTxUrl(txHash)} target="_blank" rel="noreferrer">
